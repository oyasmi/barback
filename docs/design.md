...
| D-3 | 状态栏用 AppKit（`NSStatusItem` + `NSMenu`），窗口用 SwiftUI | 菜单需区分左右键并精确控制子菜单，AppKit 可控且省资源；表单类界面 SwiftUI 效率高 | SwiftUI `MenuBarExtra`：无法区分左右键 |
...
| 运行时 | command / use_shell / directory / env / 日志路径 / stop_* / timeout | 未运行则下次启动生效；运行中则置 `needs_restart`，菜单显示「配置已变更，需重启生效」，提供「保存并重启」 |
...
SwiftUI `MenuBarExtra` 无法区分左右键，故用 AppKit：
...
@objc func handleClick(_ sender: NSStatusBarButton) {
    ...
    let menu = isRight ? appMenu : programMenu
    rebuild(menu)                 // 打开瞬间才构建，平时不维护菜单对象
    statusItem.menu = menu
    statusItem.button?.performClick(nil)
    statusItem.menu = nil         // 用完摘除，保证下次点击仍走 action
}
```

`NSMenuDelegate` 的 `menuWillOpen` / `menuDidClose` 驱动"刷新与采样"的开关：**菜单关闭时不采样 CPU/RSS、不刷新计时**，这是常态零开销的关键。

### 6.3 左键菜单

```
┌──────────────────────────────────────────────┐
│ 3 运行 · 1 停止 · 1 失败                      │ ← 摘要（禁用）
├─ 服务 ───────────────────────────────────────┤
│ ● nslocal            运行中  5天20小时    ▸  │
│ ⚠ tunnel             启动失败             ▸  │
│ ◐ api-server         重试中 2/3 · 12秒    ▸  │
│ ○ voice_typer_server 已停止               ▸  │
├─ 一次性命令 ─────────────────────────────────┤
│ ▶ 备份数据库    上次 ✓ 09-12 10:22 · 3.4s ▸ │
│ ◌ 清理缓存      执行中 12秒               ▸  │
├──────────────────────────────────────────────┤
│ 全部启动 / 全部停止 / 全部重启                │
├──────────────────────────────────────────────┤
...
服务子菜单                        一次性命令子菜单
 ...
 复制 PID (65772)                  ─────────────────────
 ─────────────────────            上次 ✓ 退出码 0 · 3.4s
 PID 65772 · CPU 0.3% · 48 MB      共执行 27 次
 启动于 09-06 13:33                ─────────────────────
 近 24 小时重启 0 次                编辑配置…
 ─────────────────────
 编辑配置…
 ⌥ 追加：强制终止 / 清除失败状态 / 复制启动命令
```

状态圆点**同时用形状与颜色区分**（实心绿=运行 / 半填充橙=启动中·重试中 / 空心灰=停止·退出 / 红⚠=失败），不单靠颜色传达信息。项数 > 12 时按 `group_name` 分节。
...
SwiftUI `NavigationSplitView`，最小 860×600：
...
├────────────────────────────┼───────────────────────────────────┤
│ ▾ 服务                     │ ┌ 基本 ──────────────────────────┐ │
│   ● nslocal                │ │ 名称 [nslocal]  类型 服务(只读) │ │
│   ⚠ tunnel                 │ │ 命令 [/Users/…/nslocal -c …  ] │ │
│   ○ voice_typer_server     │ │      ☐ 通过 /bin/sh 执行        │ │
│ ▾ 一次性命令               │ │ 工作目录 [/Users/oyasmi ] [选择]│ │
│   ▶ 备份数据库             │ │ 分组 [默认 ▾]  优先级 [100]     │ │
│   ▶ 清理缓存               │ └────────────────────────────────┘ │
│                            │ ┌ 启动与重启 ────────────────────┐ │
│                            │ │ ☑ 启动 Barback 时自动启动       │ │
...
- `[+▾]` 下拉选择新建"服务"或"一次性命令"；类型决定表单字段集，创建后不可改（可「复制为另一类型」）。
- 一次性命令的表单分区为：基本 / 执行（超时、执行前确认、允许并发、历史保留）/ 停止 / 环境变量 / 输出。
- 左栏拖拽排序 = 调整 `priority`；显示只读状态，**不提供日常启停按钮**（操作归状态栏）。
- 字段失焦即校验，错误就地红字；⌘S 保存、Esc 放弃；未保存时切换/关窗给确认。
...
| 偏好设置 | 三页：通用（开机自启、菜单密度、语言）/ 通知（分级开关、去抖窗口）/ 高级（历史保留、环境快照管理、日志级别、诊断包、数据库备份恢复） |
