...
SwiftUI `NavigationSplitView` 挂在 AppKit 窗口上，默认 980×680、最小 840×560。主操作放在原生工具栏，详情区是**一整页**表单，而不是切页的标签栏：

```
┌─ 配置 ──────────────────────────────────────────────────────────────────┐
│ ⊕ 新建 ▾   ⧉ 复制   🗑 删除                   🔍 搜索…      ⇅ 分组方式 ▾ │
├───────────────────────┬─────────────────────────────────────────────────┤
│ 服务                3 │  nslocal [服务] ⏻          ● 运行中  [日志][历史]│
│  ● nslocal            │  PID 4821 · up 2d3h · 分组 网络                  │
│    运行中 · PID 4821  │ ─────────────────────────────────────────────── │
│  ● tunnel             │   ┌ 命令 ─────────────────────────────────────┐ │
│    重试中 2/3 · 12 秒 │   │ 命令＊ [/usr/local/bin/nslocal -c ~/x.yml]│ │
│  ○ voice_typer_server │   │        ☐ 通过 /bin/sh 执行                │ │
│    已停止             │   │        ✓ 解析为 /usr/local/bin/nslocal    │ │
│ 一次性命令          2 │   │ 工作目录 [~/work           ] [选择…]      │ │
│  ▶ backup-db          │   └───────────────────────────────────────────┘ │
│    成功 · 1.2s        │   ┌ 标识 ─────────────────────────────────────┐ │
│  ▶ clean-cache        │   │ 名称＊ [nslocal      ]  分组 [网络     ▾] │ │
│    尚未执行           │   └───────────────────────────────────────────┘ │
│                       │   ┌ 行为 ─────────────────────────────────────┐ │
│                       │   │ ☑ 随 Barback 启动   自动重启⟨从不│仅异常│总是⟩│
│                       │   └───────────────────────────────────────────┘ │
│                       │   ┌ 环境变量（2） ───────────────────────────┐ │
│                       │   │ [KEY=VALUE 多行文本框]                    │ │
│                       │   └───────────────────────────────────────────┘ │
│                       │   ▸ 重启策略      默认 · 5 秒存活，最多重试 3 次 │
│                       │   ▸ 停止方式    ●已改动 2 项 · INT，20 秒后强杀 │
│                       │   ▸ 日志          默认 · 按 10 MB 轮转，保留 3 份│
│                       │   ▸ 其他          优先级 100 · 无备注           │
│ [+] [−]               │ ─────────────────────────────────────────────── │
│                       │ ● 有未保存的更改   [撤销] [保存并重启] [保存]   │
└───────────────────────┴─────────────────────────────────────────────────┘
...
- **参数分层**：`Program` 的近 30 个字段里，只有 `command`/`name` 是真正必填（标签旁 "必填" 徽章，值一填上就消失）；`directory`/`autostart`/`autorestart`（一次性命令是 `confirmBeforeRun`/`timeoutSeconds`）/`environment`/`groupName` 是常用字段，常驻可见；其余约 19 个字段（退避与风暴、正常退出码、停止信号与进程组、日志路径与轮转、优先级、备注……）本质上是抄自 supervisor、绝大多数场景用默认值就够的旋钮，一律折进「重启策略 / 停止方式 / 日志 / 其他」几个可折叠的 `AdvancedGroup`（一次性命令是「执行 / 停止方式 / 其他」）。`enabled` 从表单挪到标题区做成开关，不再算一个"配置项"。
- 每个 `AdvancedGroup` 默认折叠，标签行本身就是当前生效值的摘要（如「默认 · 5 秒存活，最多重试 3 次」），不展开也能读完整份配置；一旦组内有字段偏离 `Program` 的内置默认值，摘要前缀变成「已改动 N 项」且该字段旁出现一个小圆点，"恢复默认值"操作本身则放在展开后的内容里（避免和整行的展开/折叠点按区域打架）。组内有校验错误时强制展开，与折叠状态无关。
- 详情区结构：**身份与实况**（名称、类型徽章、启用开关、状态胶囊、PID/运行时长、跳转日志与历史，运行中且刚编辑了「运行时字段」——命令/目录/环境/日志路径/停止信号/强杀等待/超时——会在标题下出现「需要重启才能生效」提示条，带「立即重启」）→ **一整页表单**（命令 → 标识 → 行为 → 环境变量 → 折叠的高级分区）→ **提交条**（脏标记 + 撤销 + 保存 + 保存并重启）。
- 表单用 `Form.formStyle(.grouped)`，限宽约 680pt 避免在宽窗口下被拉得过宽。
- 左栏按「类型 / 分组 / 状态 / 名称」分组；仅在按类型且未搜索时允许拖拽排序 = 调整 `priority`。显示只读状态，**不提供日常启停按钮**（操作归状态栏），状态圆点的颜色/形状取自与状态栏面板共用的 `StatusStyle`，不单独维护一份映射。
